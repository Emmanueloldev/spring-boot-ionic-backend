...
public class BR {
	...
	private static final int[] weightTin = {11, 10, 9, 8, 7, 6, 5, 4, 3, 2};
	...
	public static boolean isValidCPF(final String ssn) { // isValidCPF valida um determinado CPF
	...
	public static boolean isValidCNPJ(final String tin) { // isValidCNPJ valida um determinado CNPJ
		if ((tin == null) || (tin.length() !=11) || tin.matches(tin.charAt(0) + "{11}")) return false;
		
		final Integer digit1 = calculate(tin.substring(0, 9), weightSsn);
		final Integer digit2 = calculate(tin.substring(0, 9) + digit1, weightSsn);
